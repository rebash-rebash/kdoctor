...
from tests.helpers import (configmap_key_ref, container, deployment, env,
                           replicaset, secret_key_ref)
...
class DeploymentDriftTest(unittest.TestCase):
    def test_compare_live_to_replicaset_detects_core_drift_fields(self):
        expected_container = container(
            ...
            env=[
                env("MODE", "stable"),
                env(
                    "CONFIG_VALUE", value_from=configmap_key_ref("api-config", "value")
                ),
        ...
        live_container = container(
            ...
            env=[
                ...
                env(
                    "CONFIG_VALUE",
                    value_from=configmap_key_ref("api-config-v2", "value"),
                ),
    ...
    def test_compare_live_to_replicaset_detects_missing_container(self):
        findings = deployment_drift.compare_live_to_replicaset(
            deployment(containers=[container(name="api")]),
            replicaset(containers=[container(name="api"), container(name="worker")]),
        )
