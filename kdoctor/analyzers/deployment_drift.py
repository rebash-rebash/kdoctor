...
def detect_deployment_drift(
    ...
    try:
        ...
        console.print(
            f"[red]Failed to detect deployment drift:[/red] {e}"
        )
        return

    if not replicasets:
        console.print(
            "[yellow]No ReplicaSets found for deployment[/yellow]"
        )
        return

    expected = replicasets[0]
    findings = compare_live_to_replicaset(
        deployment,
        expected
    )
...
def compare_live_to_replicaset(deployment, replicaset):
    ...
    for name in sorted(
        set(live_containers.keys())
        | set(expected_containers.keys())
    ):
...
def print_drift_report(
    ...
    console.print(
        f"Expected Source: ReplicaSet {replicaset_name}"
    )
