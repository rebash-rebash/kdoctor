...
def audit_deployment(
    ...
    try:
        ...
        console.print(f"[red]Failed to audit deployment:[/red] {e}")
        return
...
def evaluate_deployment(deployment):
    ...
    for container in deployment.spec.template.spec.containers:
        ...
        add_finding(findings, risk, not requests, 8, container.name, "Missing requests")
        add_finding(findings, risk, not limits, 8, container.name, "Missing limits")
        add_finding(
            findings,
            risk,
        ...
        add_finding(
            findings,
            risk,
            security_context and security_context.privileged is True,
            15,
            container.name,
...
def add_finding(findings, risk, condition, points, container, message):
    ...
    findings.append({"container": container, "finding": message, "points": points})
...
def runs_as_root(container_context, pod_context):
    ...
    if container_context and container_context.run_as_user is not None:
...
def print_audit_report(deployment, namespace, result):
    score = result["score"]
    console.print(
        f"[bold blue]Score:[/bold blue] " f"[{score_style(score)}]{score}/100[/]"
    )
    console.print(f"[bold blue]Risk:[/bold blue] {risk_from_score(score)}")

    table = Table(title=f"Deployment Audit ({namespace}/{deployment.metadata.name})")
    table.add_column("Container")
    table.add_column("Finding")
    table.add_column("Penalty")

    for finding in result["findings"]:
        table.add_row(finding["container"], finding["finding"], str(finding["points"]))
