...
def analyze_deployment(deployment_name, namespace, output_format: Optional[str] = None):
    ...
    pods = core.list_namespaced_pod(namespace, label_selector=selector_string)
    ...
    for pod in pods.items:
        ...
        pod_score, _ = calculate_health_score(pod)

        pod_scores.append(pod_score)

    avg_score = sum(pod_scores) // len(pod_scores) if pod_scores else 0

    final_score = (score + avg_score) // 2
    ...
    table = Table(title="Deployment Analysis")
    ...
    table.add_row("Available Replicas", str(available))
    ...
    console.print(f"Health Score: {final_score}")

    console.print(f"Risk: {get_risk(final_score)}")


def analyze_all_deployments(namespace, output_format: Optional[str] = None):
    apps = get_apps_v1()
    ...
    table = Table(title=f"Deployment Health Report ({namespace})")
    ...
    for item in results:
        ...
        table.add_row(item["name"], item["ready"], str(item["score"]), item["risk"])
