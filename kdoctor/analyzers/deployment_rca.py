...
from kdoctor.utils.kube import (configmap_refs, deployment_pods,
                                deployment_replicasets, is_crash_looping,
                                oom_events, revision_label, secret_refs,
                                total_restarts)
...
def analyze_deployment_rca(deployment_name: str, namespace: str):
    try:
        ...
        events = list_deployment_events(core, deployment_name, namespace)
        log_signals = collect_log_signals(core, pods, namespace)
    except Exception as e:
        console.print(f"[red]Failed to run RCA:[/red] {e}")
        return

    candidates = build_root_cause_candidates(pods, replicasets, events, log_signals)

    print_rca_report(deployment_name, namespace, replicasets, candidates)
...
def list_deployment_events(core, deployment_name, namespace):
    ...
    for event in response.items:
        ...
        if deployment_name in message or involved.name == deployment_name:
            events.append(event)
...
def collect_log_signals(core, pods, namespace):
    signals = {"timeout": 0, "connection": 0, "exception": 0, "permission": 0}
...
def build_root_cause_candidates(pods, replicasets, events, log_signals):
    candidates = []
    ...
    event_text = " ".join(f"{event.reason} {event.message}" for event in events).lower()

    if ooms:
        candidates.append(("Memory exhaustion", min(95, 70 + ooms * 5)))

    if crashloops:
        candidates.append(("Application crash loop", min(90, 65 + crashloops * 10)))

    if restarts and "back-off" in event_text:
        candidates.append(("Container repeatedly failing after startup", 80))
    ...
    if len(replicasets) >= 2:
        ...
        if resource_limits(current) != resource_limits(previous):
...
def print_rca_report(deployment_name, namespace, replicasets, candidates):
    console.print("[bold cyan]Root Cause Candidates[/bold cyan]")
    console.print(f"Deployment: {namespace}/{deployment_name}")

    if replicasets:
        console.print(f"Current Revision: {revision_label(replicasets[0])}")
    ...
    for index, (candidate, confidence) in enumerate(candidates, start=1):
        table.add_row(str(index), candidate, f"{confidence}%")
