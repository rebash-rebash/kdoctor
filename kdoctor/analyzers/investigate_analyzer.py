...
def analyze_container_status(
    ...
    console.print(
        "[bold cyan]Container Analysis[/bold cyan]"
    )
    ...
    for status in statuses:
        ...
        if status.restart_count > 0:
            ...
            if last_terminated:
                console.print(
                    f"Previous Exit Code: {last_terminated.exit_code}"
                )
                if last_terminated.exit_code == 137:
                    console.print(
                        "[red]OOMKilled detected[/red]"
                    )

                    console.print(
                        "Likely Cause: Container exceeded memory limit"
                    )
        ...
        if waiting:
            ...
            if reason == "CrashLoopBackOff":
                ...
                console.print(
                    "[red]CrashLoopBackOff detected[/red]"
                )
                ...
                console.print(
                    "[red]Image Pull Failure[/red]"
                )

                console.print(
                    "Likely Cause: Invalid image or registry access issue"
                )
                ...
                console.print(
                    "[red]Container Configuration Error[/red]"
                )

                console.print(
                    "Likely Cause: Missing Secret or ConfigMap"
                )

        if terminated:

            console.print(
                f"Last Exit Code: {terminated.exit_code}"
            )

            console.print(
                f"Termination Reason: {terminated.reason}"
            )
...
def analyze_events(
    ...
    try:
        ...
        console.print(
            f"[red]Failed to fetch events:[/red] {e}"
        )
    ...
    console.print()
    console.print(
        f"[bold cyan]Events ({len(events.items)})[/bold cyan]"
    )
...
def analyze_resources(pod):
    ...
    for container in pod.spec.containers:
        ...
        if not limits:
            console.print(
                f"[yellow]{container.name} missing resource limits[/yellow]"
            )
        table.add_row(
            container.name,
            requests.get("cpu", "-"),
...
def analyze_logs(
    ...
    for container in pod.spec.containers:
        ...
        for finding in findings:
            ...
            console.print(
                f"[yellow]Root Cause Candidate[/yellow]"
            )
    ...
    if not found_anything:
        ...
        console.print(
            "No obvious failure patterns detected"
        )
